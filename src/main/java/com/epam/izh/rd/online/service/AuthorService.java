 ...
 * 4) Добавить в SimpleAuthorService приватное поле "AuthorRepository authorRepository" - это репозиторий к которому
 * вы будете обращаться в методах
 * 5) Создать дефолтный конструктор (без параметров)
 * 6) Создать конструктор с параметром AuthorRepository authorRepository (который будет устанавливать в поле authorRepository значение)
...
public interface AuthorService {
     ...
     * По факту, он просто обращается к репозиторию с авторами и вызывает аналогичный метод, после чего возвращает результат.
     */
