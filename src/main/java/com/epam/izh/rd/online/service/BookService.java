 ...
 * 6) Создать дефолтный конструктор (без параметров)
 * 7) Создать конструктор с параметрами "BookRepository<SchoolBook> schoolBookBookRepository, AuthorService authorService"
 * (который будет устанавливать в поле schoolBookBookRepository и в поле authorService значения)
...
public interface BookService<T extends Book> {
     ...
     * То есть он должен взять имя и фамилию автора из книги и обратиться к сервису авторов и узнать о наличии такого автора.
